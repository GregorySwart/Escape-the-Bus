...
class Game:
    ...
    strategy: GameStrategy = GameStrategy.SENSIBLE

    def __init__(self):
        for suit in range(4):
            for number in range(13):
                self.cards.append(PlayingCard(number + 1, suit + 1))
        self.strategy = GameStrategy.SENSIBLE
    ...
    def play_first_card(self) -> Tuple[PlayingCard, str, bool]:
        if self.strategy in [GameStrategy.RANDOM, GameStrategy.SENSIBLE]:
    ...
    def play_second_card(self, first_card: PlayingCard) -> Tuple[PlayingCard, bool, bool]:
        if self.strategy == GameStrategy.RANDOM:
            guess = bool(randint(0, 1))
        elif self.strategy == GameStrategy.SENSIBLE:
    ...
    def play_third_card(self, first_card: PlayingCard, second_card: PlayingCard) -> Tuple[PlayingCard, bool, bool]:
    ...
    def play(self, strategy: Optional[GameStrategy] = None) -> Tuple[bool, int]:
        self.deal_new_deck()
        self.shuffle_deck()
        ...
        while not escape:
            try:
                first_card, guess, success_first = self.play_first_card()
                if not success_first:
                    continue

                second_card, guess, success_second = self.play_second_card(first_card)
                if not success_second:
                    continue

                third_card, guess, success_third = self.play_third_card(first_card, second_card)
...
def main():
    new_game = Game()
    new_game.strategy = GameStrategy.OPTIMISED
    outcomes = []
    card_counter = []
    for i in range(100):
        print(f"Playing game {i + 1}...")
        outcome, n_cards = new_game.play()
